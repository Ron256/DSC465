...
```
```{r}
unique(perceptionExperiment$Test)
...
perceptionExperiment <- perceptionExperiment %>% 
  mutate(Test = factor(Test, 
                          levels = c( 'Slope', 'Color Value', 'Volume', 'Area', 'Length, Non-Aligned', "Veritcal Distance, Aligned",'Vertical Distance, Non-Aligned', 'Angle')
                          )
         ) #%>% 
 #mutate(results = fct_reorder(Test, absoluteError)) # ordering the data

p <- ggplot(perceptionExperiment, aes(x = fct_reorder(Test, desc(absoluteError)), y = absoluteError)) +
  geom_col() +
  expand_limits(y=0) + # Make the y-axis go to zero.
  theme_grey(base_size = 14) +
  theme(axis.text.x = element_text(face="bold.italic",
